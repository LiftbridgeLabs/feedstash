"""API tokens for the extension, email worker and phone apps."""
...
from app.web.deps import DatabaseDep, UserDep, api_token_id
...
def list_tokens(user: UserDep, db: DatabaseDep) -> list[TokenOut]:
...
def create_token(body: NewTokenIn, user: UserDep, db: DatabaseDep) -> NewTokenOut:
    with db.transaction() as conn:
...
def revoke_token(token_id: int, request: Request, user: UserDep, db: DatabaseDep) -> Response:
